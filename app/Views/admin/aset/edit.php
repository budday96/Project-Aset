...
<div class="card">
    <div class="card-body">
        ...
        // $cabangs: list cabang (untuk admin)
        ?>

        <form action="<?= base_url('admin/aset/update/' . (int)$row['id_aset']) ?>" method="post" enctype="multipart/form-data" class="needs-validation" novalidate>
            <?= csrf_field() ?>
...
<script>
    (() => {
        ...
        async function loadMasterDetail(id) {
            ...
            try {
                const res = await fetch(`${BASE}/admin/aset/ajaxMasterDetail/${id}`);
