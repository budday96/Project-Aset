...
<div class="card">
    <div class="card-body">
        <form action="<?= base_url('admin/master-aset/update/' . (int)$row['id_master_aset']) ?>" method="post" class="needs-validation" novalidate>
            <?= csrf_field() ?>
            ...
            <div class="row g-3">
                ...
                <div class="col-md-6">
                    ...
                    <input type="text"
                        ...
                        value="<?= old('nama_master', $row['nama_master'] ?? '') ?>"
                        required>
                ...
                <div class="col-md-3">
                    ...
                    <input type="date"
                        ...
                        value="<?= old('expired_default', $row['expired_default'] ?? '') ?>">
                    <div class="invalid-feedback">
                ...
                <div class="col-md-6">
                    ...
                    <input id="nilai_perolehan_default" type="hidden" name="nilai_perolehan_default"
                        value="<?= esc(old('nilai_perolehan_default', $row['nilai_perolehan_default'] ?? '')) ?>">
                    <div class="invalid-feedback">
                ...
                <div class="col-md-6">
                    ...
                    <input type="month"
                        ...
                        value="<?php
                                $pm = old('periode_perolehan_default_month');
                                if (!$pm && !empty($row['periode_perolehan_default'])) {
                                    $pm = date('Y-m', strtotime($row['periode_perolehan_default']));
                                }
                                echo esc($pm ?? '');
                                ?>">
                    <div class="invalid-feedback">
                ...
                <div class="col-md-6">
                    ...
                    <select id="id_kategori" name="id_kategori"
                        ...
                        <option value="" disabled <?= old('id_kategori', $row['id_kategori'] ?? '') ? '' : 'selected' ?>>-- Pilih --</option>
                        <?php foreach ($kategoris as $k): ?>
                            <option value="<?= $k['id_kategori'] ?>"
                                <?= (string)old('id_kategori', $row['id_kategori'] ?? '') === (string)$k['id_kategori'] ? 'selected' : '' ?>>
                                <?= esc($k['nama_kategori']) ?>
                            </option>
                        <?php endforeach ?>
                    </select>
            ...
            <div class="mt-3">
                ...
                <a href="<?= base_url('admin/master-aset') ?>" class="btn btn-secondary">Kembali</a>
            </div>
        </form>
    </div>
</div>

<!-- JS sama seperti create, namun prefill old() + nilai default -->
<script>
    (() => {
        const BASE = '<?= base_url() ?>';
        ...
        async function loadSub(kid, pre = null) {
            ...
            try {
                const r = await fetch(`${BASE}/admin/subkategori/by-kategori/${kid}`);
        ...
        async function loadAttrs(sid) {
            ...
            try {
                const r = await fetch(`${BASE}/admin/atribut/by-subkategori/${sid}`);
                ...
                // nilai lama (old) + nilai default yang sudah tersimpan di master ini
                const preset = <?= json_encode(old('default_atribut', [])) ?>;

                let defaults = {};
                <?php if (!empty($row)): ?>
                    try {
                        const preR = await fetch(`${BASE}/admin/aset/ajaxMasterDetail/<?= (int)$row['id_master_aset'] ?>`);
                        const preJ = await preR.json();
                        if (preJ && preJ.ok && Array.isArray(preJ.atribut)) {
                            defaults = preJ.atribut.reduce((acc, a) => {
                                acc[String(a.id_atribut)] = a.nilai;
                                return acc;
                            }, {});
                        }
                    } catch (e) {
                ...
                attrs.forEach(a => {
                    ...
                    const preVal = (preset[String(a.id_atribut)] ?? defaults[String(a.id_atribut)] ?? '');
        ...
        // Restore untuk edit / old()
        (function restoreKS() {
            const OLD_K = '<?= old('id_kategori', $row['id_kategori'] ?? '') ?>';
            const OLD_S = '<?= old('id_subkategori', $row['id_subkategori'] ?? '') ?>';
            if (OLD_K) {
                selKat.value = OLD_K;
        ...
        (function initRupiah() {
            ...
            inputNilai.addEventListener('input', function() {
                const before = this.value,
                    caret = this.selectionStart || before.length;
            ...
            inputNilai.addEventListener('blur', () => {
                if (inputNilai.value.trim() === '') hiddenNilai.value = '';
            });
        })();
