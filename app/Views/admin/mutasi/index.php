    ...
    <div class="card shadow-sm border-0 mb-4">
        <div class="card-header bg-white py-3 px-4 d-flex flex-column flex-md-row align-items-start align-items-md-center justify-content-between gap-3">
            ...
            <div class="d-flex flex-wrap gap-2">
                ...
                <a href="<?= base_url('/admin/mutasi'); ?>" class="btn btn-outline-secondary btn-sm" data-bs-toggle="tooltip"
                    data-bs-placement="top" title="Refresh">
                    <i class="bi bi-arrow-clockwise fs-5"></i>
                </a>
                <a class="btn btn-warning btn-sm fw-semibold d-flex align-items-center" href="<?= base_url('admin/mutasi/create') ?>">
                ...
                <a class="btn btn-dark btn-sm fw-semibold d-flex align-items-center" data-bs-toggle="modal" data-bs-target="#view-notes" href="#">
                    ...
                    <span class="d-none d-sm-inline">Import Mutasi</span>
                </a>
            </div>
        </div>
                                ...
                                <div class="list-group-item bg-white rounded-3 shadow-sm mb-3 border-0 px-4 py-3">
                                    ...
                                    <div>
                                        <?php if (($m['status'] ?? '') === 'pending'): ?>
                                            <form action="<?= base_url('admin/mutasi/kirim/' . ($m['id_mutasi'] ?? '')); ?>" method="post" class="d-inline">
                                                <?= csrf_field(); ?>
                                                <button type="submit" class="btn btn-sm btn-info me-1" onclick="return confirm('Kirim mutasi ini?');">
                                                    <i class="fas fa-paper-plane me-1"></i> Kirim
                                                </button>
                                            </form>
                                            <form action="<?= base_url('admin/mutasi/batalkan/' . ($m['id_mutasi'] ?? '')); ?>" method="post" class="d-inline">
                                                <?= csrf_field(); ?>
                        ...
                        <div class="list-group list-group-flush">
                            <?php foreach ($masuk as $m): ?>
                                ...
                                <div class="list-group-item bg-white rounded-3 shadow-sm mb-3 border-0 px-4 py-3">
                                    <div class="d-flex w-100 justify-content-between align-items-center mb-2">
                                        <div>
                                            <h5 class="mb-1 fw-bold text-success"><?= esc($m['nama_aset'] ?? '-'); ?></h5>
                                        </div>
                                        <span class="badge bg-<?= $badge; ?> text-uppercase px-3 py-2 fs-7"><?= esc($m['status'] ?? '-'); ?></span>
                                    </div>
                                    ...
                                    <div>
                                        <?php if (in_array($m['status'] ?? '', ['pending', 'dikirim'], true)): ?>
                                            <form action="<?= base_url('admin/mutasi/terima/' . ($m['id_mutasi'] ?? '')); ?>" method="post" class="d-inline">
                                                <?= csrf_field(); ?>
