...
<div class="card">
    <div class="card-body">
        ...
        // $cabangs: list cabang (untuk user)
        ?>

        <form action="<?= base_url('user/aset/update/' . (int)$row['id_aset']) ?>" method="post" enctype="multipart/form-data" class="needs-validation" novalidate>
            <?= csrf_field() ?>
...
<script>
    (() => {
        ...
        async function loadMasterDetail(id) {
            ...
            try {
                const res = await fetch(`${BASE}/user/aset/ajaxMasterDetail/${id}`);
